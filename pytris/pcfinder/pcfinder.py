...
class PCFinder:
    ...
    def generate_possible_queue_combinations(self, queue: Queue, res_size: int) -> Dict[str, List[Queue]]:
        """
        ...
        if self.no_hold:
            ...
            return {str(queue[res_size]): [queue[:res_size]]}

        # left_queue, hold, result_queue
        working_nodes = [[queue[:], [], []]]
        ...
        while True:
            ...
            if len(node[2]) == res_size:
                save = "" if not node[1] else str(node[1][0])
                if save not in to_return_dict:
                    to_return_dict[save] = []
                if node[2] not in to_return_dict[save]:
                    to_return_dict[save] += [node[2]]
                continue
            res = []
            if node[0]:
                piece = node[0][0]
    ...
    def apply_in_grid(self, grid_state: BoolGrid, pos: PiecePos, clear: bool = True) -> Tuple[BoolGrid, bool]:
        ...
        if clear:
            ...
            for cell in pos:
                current_line = cell[0]
                while new_grid_state[current_line][cell[1]]:
                    current_line += 1
                new_grid_state[current_line][cell[1]] = True
    ...
    def useless_filled_cells(self, grid_state: BoolGrid, line_nb: int, starting_col: int, end_col: int) -> int:
        filled_to_ignore = 0
        for col in range(starting_col, end_col // 2):
            if sum(line[col] for line in grid_state) < line_nb:
                break
            filled_to_ignore += line_nb
        for col in reversed(range(starting_col, end_col)):
            if sum(line[col] for line in grid_state) < line_nb:
                break
            filled_to_ignore += line_nb
        return filled_to_ignore

    def box_unbuildable(self, queue: Queue, cells_to_count: int) -> bool:
        if cells_to_count == 0:
            oi_sum = sum(piece in (I_PIECE, O_PIECE) for piece in queue)
            ljt_sum = sum(piece in (L_PIECE, J_PIECE, T_PIECE) for piece in queue)
            return oi_sum < len(queue) and ljt_sum < 2
        return False

    def solve(self, queue: Queue, grid_state: BoolGrid, collect: Dict[str, List[Tuple[Queue, List[PieceMov]]]],
              no_split: bool = False, return_first: bool = True) -> Dict[str, List[Tuple[Queue, List[PieceMov]]]]:
        """
        ...
        if no_split:
            for save, queues in self.generate_possible_queue_combinations(queue, needed_pieces).items():
                for queue_with_hold in queues:
                    if filled_cell == 0:
                    ...
                    if res:
                        if save not in collect:
                            collect[save] = []
                        for sol in res:
                            collect[save].append((queue_with_hold, sol))
                        if return_first:
                            print(time.perf_counter() - before)
                            return collect
                        print(f"Found save {save} solutions : {collect[save]}")
                        break
            print(time.perf_counter() - before)
            return collect
                            ...
                            for sub_sol in sub_sols:
                                ...
                                for sol in full_solve:
                                    ...
                                    if self.verify_solution(queue_with_hold, moves, grid_state):
                                        if save not in collect:
                                            collect[save] = []
                                        collect[save].append((queue_with_hold, moves))
                                        if return_first:
                                            print(time.perf_counter() - before)
                                            return collect
                                        found_solution = True
...
if __name__ == "__main__":
    ...
    with cProfile.Profile() as pr:
        ...
        print(f"Solve nb : {sum(len(v) for v in res.values())}")
    print(time.perf_counter() - before)
